...
async function checkDependenciesDep(id){

  const result = await $.ajax({

    url: "./libs/php/checkDependeciesDep.php",
    type: 'GET',
    dataType: 'json',
    data: {id: id},
...
async function checkDependenciesLoc(id){

  const result = await $.ajax({

    url: "./libs/php/checkDependeciesLoc.php",
    type: 'GET',
    dataType: 'json',
    data: {id: id},
    ...
    case "personnel":
      {
        ...
        $("#deleteBtn").on('click', async function(){
          ...
          $("#alert").html(elem.firstName + " " + elem.lastName + " Deleted")
    ...
    case "department":
      {
        ...
        if(elem.dependency == 0){
          $("#deleteBtn").on('click', async function(){
            ...
            //checking the dependency again just before deleting

            let dependency =  await checkDependenciesDep($(this).attr("data-id"))

            if(dependency.dependency == 0){
              await deleteDepByID($(this).attr("data-id"))
              reset().then(launchSearch())
              $("#alert").html(elem.Department + " Deleted")
              $("#alert").show().delay(3000).fadeOut();
            }else{
              reset().then(launchEdit())
              $("#alert").html(elem.Department + " has " + dependency.dependency + " dependencies")
              $("#alert").show().delay(3000).fadeOut();
            }
    ...
    case "location":{
      ...
      if(elem.dependency == 0){
        ...
        $("#deleteBtn").on('click', async function(){
          ...
          let dependency =  await checkDependenciesLoc($(this).attr("data-id"))

          if(dependency.dependency == 0){

            await deleteLocByID($(this).attr("data-id"))
            reset().then(launchSearch())
            $("#alert").html(elem.Location + " Deleted")
            $("#alert").show().delay(3000).fadeOut();

          }else{

            reset().then(launchEdit())
            $("#alert").html(elem.Location + " has " + dependency.dependency + " dependencies")
            $("#alert").show().delay(3000).fadeOut();
    ...
    case "personnel":
      {
        ...
        $("#saveCreateBtn").on('click', async function(){
          ...
          let perN = $("#inputFirstName").val() + " " + $("#inputLastName").val()   
          
          reset().then((value) => {launchSearch()})

          $("#alert").html( perN + " Inserted")
          $("#alert").show().delay(3000).fadeOut();
    ...
    case "department":
      {
        ...
        $("#saveCreateBtn").on('click', async function(){
          ...
          let depN = $("#inputDepartment").val()

          reset().then(() => {launchSearch()})

          $("#alert").html(depN + " Inserted")
          $("#alert").show().delay(3000).fadeOut();
        })
    ...
    case "location":
      {
        ...
        $("#saveCreateBtn").on('click', async function(){
          ...
          let locN = $("#inputLocation").val()
          reset().then(() => {launchSearch()})

          $("#alert").html(locN + " Inserted")
